...
function Card({ product, onClick }) {
  ...
  const handleMoveDetail = () => {
    makeRequest(product.url, {});
  };
  return (
    <StyledCardDiv onClick={handleMoveDetail}>
      <StyledImgDiv>
        <Coupon value={product.sale} disable={product.disable} />
        <StyledImg src={product.img} />
      </StyledImgDiv>
      <StyledDetailDiv>
        <GetCartBtn type="button" size="ml" value="담기" onClick={onClick} />
      ...
      <ReviewDiv>
        ...
        <ReviewSpan>{product.review}</ReviewSpan>
      </ReviewDiv>
    </StyledCardDiv>
  );
}
